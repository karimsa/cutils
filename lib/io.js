...
module.exports = function (sol) {
  ...
  // duplicate the format if need be
  sol.format = dup(sol.format, sol.cases)
  ...
  fs.readFile(path.resolve(sol.dir, sol.file), 'utf8', function (err, data) {
    ...
    for (x = 0, z = 0, acase = [], line = []; x < data.length; x += 1, z += 1, line = []) {
      for (y = 0; y < sol.format[z].length && data[x].length > 0; y += 1) {
        ...
        } else if (sol.format[z][y].match(/[0-9]+\:/)) {
          var N = parseInt(sol.format[z][y].substr(0, sol.format[z][y].length - 1), 10)

          for (i = 0; i < N; i += 1) {
            sol.format.push(sol.format[z].slice(y + 1))
          }

          y = sol.format[z].length
          x -= 1
        } else {
          t = data[x].indexOf(' ')
    ...
    sol.logic.on('end', function () {
      ...
      evts.emit('end', time > 60000 ? (new Error('solution took too long')) : null)
